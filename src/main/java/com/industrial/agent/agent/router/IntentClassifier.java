...
public class IntentClassifier {
    ...
    private static final Pattern DIAGNOSIS_PATTERN = Pattern.compile("诊断|故障|排查|检修|工单|维修|停机|异常.*分析");
    ...
    private Intent classifyByLlm(String message) {
        try {
            String prompt = "将以下用户消息分类为一个类别，只返回类别名称，不要其他内容。\n" +
                    "类别：ALARM（告警查询）、DATA（数据查询）、DIAGNOSIS（故障诊断）、KNOWLEDGE（知识检索）、GENERAL（一般对话）\n" +
                    "消息：" + message;
